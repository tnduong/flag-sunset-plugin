...
12. Using only the main agent, run exact local usage discovery for the candidate apps with extension-filtered terminal search commands and build the concrete future work set:
   ...
   - emit debug evidence lines defined in [search-strategy.md](./search-strategy.md#step-2---read-the-exact-constant-name) and [search-strategy.md](./search-strategy.md#step-3---local-usage-search) so each run captures app roots, symbol-level counts, second-hop coverage, and future-work membership
