...
6. Derive the effective local project path for every project row: local repository root + `Path in Repo` (if `Path in Repo` is `./`, use the local repository root itself).
7. Compare those derived project paths against the active VS Code workspace folders; do not use parent repository roots for workspace-gate filesystem reads.
...
- Treat any canceled, dismissed, timed-out, or interrupted validation or read as `STEP_1_INCOMPLETE` and print one blocked-item line: `Blocked item: [tool] [target]`.
...
- Print the status line after root validation, after the candidate app set is confirmed, after the concrete future work set is fully approved, and on interruption. On interruption, report the exact blocked item, repeat the latest status line verbatim, and stop.
...
2. Read [applications.md](../applications.md).
3. Resolve the local repository roots for the current run from the workspace-local `.copilot/flag-sunset/local-roots.json` under the `Nova` workspace folder. If missing, unreadable, invalid JSON, or missing required keys, continue with Preflight item 4. Do not read local-roots config from any other location.
...
7. Confirm every effective app path is already present in the active VS Code workspace before any VS Code filesystem or search tool runs; if any required effective app path is missing, stop with workspace-gate failure instead of attempting external reads.
8. Refresh local `main` from `origin/main` for each unique repository listed in [applications.md](../applications.md) before discovery:
   - run serially, one repository at a time, using a fast-forward-only update policy
   - prevent local Husky post-merge hooks from blocking the gate by overriding `core.hooksPath` for the merge command only
   - for each repository root, run these serial git commands (identical on all platforms):
   ...
   - treat successful completion of all four commands as success evidence; if one fails, use its label (`rev-parse`, `fetch`, `switch-main`, or `merge-ff-only`) as the gate-failure reason
...
9. If the main freshness gate passed, validate working-tree cleanliness for each unique repository listed in [applications.md](../applications.md) before discovery:
   - use OS-appropriate terminal git status commands on the resolved repository roots, serially, evaluating staged, unstaged, and untracked changes
...
12. Using only the main agent, run exact local usage discovery for the candidate apps with `grep_search` and build the concrete future work set (definition files; usage files that may be edited; spec/test/mock files only if proven relevant; files for later `get_errors` checks in Step 5).
   - Apply the [Canonical Search Rules](./search-strategy.md#canonical-search-rules) for tool choice, scope, extension filters, and maxResults policy. For Angular apps, include `**/*.spec.ts` only when proven relevant.
   - Apply the downstream-symbol second-hop rule from [search-strategy.md](./search-strategy.md) when building the concrete future work set.
   - If a candidate Angular source file is expected to lose a cleanup-only library import/provider during flag removal, include the co-located `*.spec.ts` file in the concrete future work set for mirrored cleanup review.
...
13. Read each file in the concrete future work set with `read_file` to trigger any remaining file-scoped approvals:
   ...
   - The `grep_search`-discovered line numbers from item 12 are the authoritative completeness list. Every matched line must fall within a read range. If any falls outside, expand the nearest range to include it.
   - After each permission-bearing tool call, continue on success or stop and print the blocked item and Step 1 status on interruption.
   - If the user approves after interruption, retry that exact `read_file` call once before doing anything else. If the same read is interrupted again, stop and ask whether to retry or abort.
